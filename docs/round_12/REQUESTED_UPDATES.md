...
### REQUESTS
- [ ] Refactor typed output registration and step configuration to reduce error-prone string usage and automate registration:
    - Implement an object or class for each tool step (e.g., `PaletteExtractionOutputStep`) that:
        - Holds the step name (e.g., 'extract_palette') as a static property or constant
        - Provides the `fromMap` factory method for output construction
        - Contains the output schema definition
        - Encapsulates the output type
    - Update the workflow and registry logic so that:
        - Each `ToolCallStep` or its `StepConfig` accepts the new step object/class directly
        - Registration of typed outputs in `ToolOutputRegistry` is automated based on the provided step objects/classes at the step level
